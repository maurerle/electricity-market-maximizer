...
def statTest(data):
    ...
    plt.savefig('rollingD1.png', transparent=True)
    plt.close()
...
plt.savefig('pacf.png', transparent=True)
plt.close()
# -
...
for p in p_val:
    for q in q_val:
        ...
        mape[p_val[p]][q_val[q]] = np.mean(np.abs((np.asarray(y) - np.asarray(y_hat)) / np.asarray(y))) * 100
# -
...
plt.savefig('heatmap_1day_mse.png', transparent=True)
plt.close()
...
ax.set_ylabel('Autoregressive Order (p)')
ax.set_xlabel('Moving Average Order (q)')
plt.savefig('heatmap_8day_mse.png', transparent=True)
plt.close()
