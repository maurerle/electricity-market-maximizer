...
class Genetic():
	...
	def getData(self, market):
		...
		res = (
			self.client
			.query(f"SELECT * FROM demand{market} WHERE time = '{targetDay}'")
			.raw
		)
		...
		res = (
			self.client
			.query(f"SELECT * FROM supply{market} WHERE time = '{targetDay}'")
			.raw
		)
		sup =(
			pd
	...
	def run(self):
		...
		while not self.stop:
			...
			while max(self.best_out) >= np.max(fitness) and not self.stop:
				...
				if n_it == STOP:
					self.stop = True
					break
				# If the optimum is found, stop the loop
				if max(self.best_out)==np.max(fitness):
					break
			...
			if generation >= 1:
				obj = {
					'x': generation,
					'y': round(self.best_out[generation]/1000.0, 2)
				}
				profit.append(obj)
			print(self.best_out)
		...
		return json.dumps(profit), new_pop[best_match,:]
