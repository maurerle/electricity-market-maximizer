	...
	def getFitness(self, pop):
		...
		for individual in pop:
			...
			try:
				...
				profit_mgp = self.getProfit1(self.sup1, self.dem1, pun1)
				self.profit_mgp.append(profit_mgp)
				profit += profit_mgp
			except IndexError:
				profit -= 1e8
				self.profit_mgp.append(0)
			try:
				pun2 = self.computeClearing1(self.sup2, self.dem2)
				profit_mi = self.getProfit1(self.sup2, self.dem2, pun2)
				self.profit_mi.append(profit_mi)
				profit += profit_mi
			except IndexError:
				profit -= 1e8
				self.profit_mi.append(0)
			try:
				pun3d, pun3s = self.computeClearing2(self.sup3, self.dem3)
				profit_msd = self.getProfit2(self.sup3, self.dem3, pun3s, pun3d)
				self.profit_msd.append(profit_msd)
				profit += profit_msd
			except IndexError:
				profit -= 1e8
				self.profit_msd.append(0)
